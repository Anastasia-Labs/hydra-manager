...
export class HydraWrapper {
   ...
   */
  getProtocolParameters = () => tryPromiseWithErrorHandling(
   ...
   */
  getUtxos = (addressOrCredential: Address | Credential) => tryPromiseWithErrorHandling(
   ...
   */
  getUtxosWithUnit = (addressOrCredential: Address | Credential, unit: Unit) => tryPromiseWithErrorHandling(
   ...
   */
  getUtxoByUnit = (unit: Unit) => tryPromiseWithErrorHandling(
    () => this.hydra.getUtxoByUnit(unit),
    `retrieving UTxO by unit ${unit}`,
    [
      {
        condition: (error) => error.message.includes("UTxO with unit not found"),
   ...
   */
  getUtxosByOutRef = (outRefs: Array<OutRef>) => tryPromiseWithErrorHandling(
   ...
   */
  getDelegation = (rewardAddress: RewardAddress) => tryPromiseWithErrorHandling(
    () => this.hydra.getDelegation(rewardAddress),
    `retrieving delegation for ${rewardAddress}`,
    [
      {
        condition: (error) => error.message.includes("Method not implemented"),
   ...
   */
  getDatum = (datumHash: DatumHash) => tryPromiseWithErrorHandling(
   ...
   */
  awaitTx = (txHash: TxHash, checkInterval?: number) => tryPromiseWithErrorHandling(
   ...
   */
  submitTx = (tx: Transaction) => tryPromiseWithErrorHandling(
    () => this.hydra.submitTx(tx),
    "submitting transaction",
    [
      {
        condition: (error) => error.message.includes("Transaction is invalid"),
        message: "Invalid transaction: Unable to validate transaction"
      },
      {
        condition: (error) => error.message.includes("Error posting transaction with hash"),
   ...
   */
  evaluateTx = (tx: Transaction, additionalUTxOs?: Array<UTxO>) => tryPromiseWithErrorHandling(
