...
import { tryPromiseWithErrorHandling, tryWithErrorHandling } from "../utils/ErrorHandling.js";
...
export class NodeWrapper {
   ...
   */
  connect = () => tryPromiseWithErrorHandling(
    () => this.node.connect(),
   ...
   */
  initialize = () => tryPromiseWithErrorHandling(
   ...
   */
  commit = (utxos: UTxO[] = [], blueprintTx?: string) => tryPromiseWithErrorHandling(
   ...
   */
  cardanoTransaction = (transaction: any) => tryPromiseWithErrorHandling(
   ...
   */
  snapshotUTxO = () => tryPromiseWithErrorHandling(
   ...
   */
  protocolParameters = () => tryPromiseWithErrorHandling(
   ...
   */
  newTx = (transaction: any) => tryPromiseWithErrorHandling(
   ...
   */
  awaitTx = (txHash: string, checkInterval?: number) => tryPromiseWithErrorHandling(
   ...
   */
  close = () => tryPromiseWithErrorHandling(
    () => this.node.close(),
    "closing Hydra head",
    [
      {
   ...
   */
  fanout = () => tryPromiseWithErrorHandling(
    () => this.node.fanout(),
    ...
    [
      ...
      {
        condition: (error) => error.message.includes("Error posting transaction for command Fanout"),
   ...
   */
  getStatus = () => tryWithErrorHandling(
    () => this.node.getStatus(),
